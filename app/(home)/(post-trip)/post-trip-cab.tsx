...
export default function PostTripFormCab() {
  const dispatch = useDispatch();
  const { postTripFormData } = useSelector((state: any) => state.driver);

  const [formData, setFormData] = useState({
    allFunctioning: postTripFormData?.cab?.allFunctioning ?? true,
    windshieldClean: postTripFormData?.cab?.windshieldClean || false,
    wipersFunctional: postTripFormData?.cab?.wipersFunctional || false,
    gauges: postTripFormData?.cab?.gauges || false,
    horn: postTripFormData?.cab?.horn || false,
    defrostHeater: postTripFormData?.cab?.defrostHeater || false,
    windshieldDetails: postTripFormData?.cab?.windshieldDetails || "",
    wipersDetails: postTripFormData?.cab?.wipersDetails || "",
    gaugesDetails: postTripFormData?.cab?.gaugesDetails || "",
    hornDetails: postTripFormData?.cab?.hornDetails || "",
    defrostHeaterDetails: postTripFormData?.cab?.defrostHeaterDetails || "",
  });
  ...
  const handleCheckboxToggle = (field: string) => {
    if (field === "allFunctioning") {
      ...
      const isUnchecking = (formData as any)[field];
      const detailsField = (detailsFieldMap as any)[field];
      setFormData({
        ...formData,
